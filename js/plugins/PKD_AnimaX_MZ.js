...
/*:
 * @plugindesc (v.1.2)[BASIC] Characters animations system
...
// * Менеджер для работы с БД анимаций
//$[ENCODE]
XAnimaTools._createXAnimaSetFromParams = function(id, type, state, params) {
  var animaSet, e, filename, frames, isOneDirection, speed;
  try {
    ...
    animaSet = new XAnimaSet(type, filename, frames, speed, isOneDirection, false);
    animaSet.dx = params.dx || 0;
    animaSet.dy = params.dy || 0;
    if (params.expandFirstFrame > 0) {
      animaSet.expandFirstFrameTimes(params.expandFirstFrame);
...
//?UPD
XAnimaTools.createXAnimaSetForDashing = function(id, state, params) {
  return null;
};
...
//$[ENCODE]
(function() {
  var ALIAS__addNewXAnimPart, ALIAS__registerAnimaXAction, _;
  //@[DEFINES]
  _ = Game_Character.prototype;
  //@[ALIAS]
  ALIAS__registerAnimaXAction = _.registerAnimaXAction;
  _.registerAnimaXAction = function(actionName) {
    if (this._axAvailableActionsList.length >= 3) {
      return;
    }
    ALIAS__registerAnimaXAction.call(this, actionName);
  };
  //@[ALIAS]
  ALIAS__addNewXAnimPart = _.addNewXAnimPart;
  _.addNewXAnimPart = function(partId, isRelative) {
    if (Object.entries(this.animaXParts).length >= 3) {
      return;
    }
    ALIAS__addNewXAnimPart.call(this, partId, isRelative);
  };
  //UPD
  //$[OVER]
  _.isHaveDashAnimaX = function() {
...
//Plugin PKD_AnimaX automatic build by PKD PluginBuilder 1.9.2 29.11.2021
